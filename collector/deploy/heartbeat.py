...
today is older than --stale-min, emit an alert.
...
def newest_snapshot(s3, bucket: str, ticker: str, day) -> pd.Timestamp | None:
    """Newest cboe_delayed snapshot timestamp (UTC) for (ticker, day), or None."""
    prefix = f"{PREFIX}/source=cboe_delayed/ticker={ticker}/date={day}/"
    newest = None
