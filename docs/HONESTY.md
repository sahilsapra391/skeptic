...
- the headline verdict still comes from the FULL-history run, and only
  the ±20% probes are windowed;
...
## F5 (ENGINE-V4): displayed depth is disclosed, never modeled (yet)
...
disclosed: net_prem_ticks is the DRIVING family, and a session whose nope
...
enter the sums as the non-NaN subset (all-NaN columns yield None, so the
...
- **dolthub_vs_uw** (SPY): chain volume/OI per expiry vs UW's
  aggregates (OI band 5%, volume 10%, because capture cutoffs differ). NOTE
...
- **The fill audit is on-demand** (the replay-receipt mechanics): it
  ...
  those very prints, so they are never audited (self-confirmation is not
...
- **Small deltas sweep absolute 0.025Δ steps (2026-07-14, the PR #99
  ...
  wing (the convention this repo already encodes as skew_25d). The
  strike-granularity arithmetic above independently lands on the
